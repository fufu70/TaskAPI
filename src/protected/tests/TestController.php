...
class TestController extends CDbTestCase
{
    ...
    {
        return $this->getJSON($redirect_url, $action_name);
    }

    /**
     * Gets a clean json object from the response of an expected HTTP 424 response 
     * from the current controler.
     * 
     * @param  string $redirect_url The action called from the current controller.
     * @param  string $action_name  The name of the action to call in the controller.
     * @return stdClass             A PHP object of the JSON object.
     */
    protected function getFailJSON($redirect_url = "", $action_name = 'actionIndex') 
    {
        return $this->getJSON($redirect_url, $action_name, "HTTP/1.1 424 \n");
    }

    /**
     * Gets a clean json object from the response of an expected header from the 
     * current controler.
     * 
     * @param  string $redirect_url    The action called from the current controller.
     * @param  string $action_name     The name of the action to call in the controller.
     * @param  string $expected_header The expected header to remove from the response.
     * @return stdClass                A PHP object of the JSON object.
     */
    private function getJSON($redirect_url = "", $action_name = 'actionIndex', $expected_header = "HTTP/1.1 200 OK\n")
    {
        $_SERVER['REDIRECT_URL'] = $redirect_url;
        ob_start();
        $controller = new $this->controller_name(rand(0,1000));
        ...
        // clean json
        $json_response = str_replace($expected_header, "", $response);
